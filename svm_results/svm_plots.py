...
for i, (fvt, lab) in enumerate(zip(fvec_type, fvec_label)):
  ...
  lines = plt.plot(test_points_x, test_points_y, 'ro-', label=lab + " (test)", markersize=8)
  for line in lines:
    line.set_color(cmap(0.8 - 0.28 * i))
    line.set_linewidth(4)
  lines = plt.plot(train_points_x, train_points_y, 'ro--', label=lab + " (train)", markersize=6)
  for line in lines:
    line.set_color(cmap(0.8 - 0.28 * i))
